...
const AddQuote = () => {
  ...
  useEffect(() => {
    ...
    const fetchPolicyDetails = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:8080/api/policy/get-one/${policyId}`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.post(`http://localhost:8080/api/quote/add/proposal/${proposalId}`, formData,
        {
          headers: { 
        ...
        <div className="card-body">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="d-flex justify-content-end">
              <button type="button" className="btn btn-secondary me-2" onClick={() => navigate('/proposals')}> Cancel </button>
